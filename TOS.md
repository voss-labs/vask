...
Both run the line you type through Cloudflare Workers AI (model: `@cf/aisingapore/gemma-sea-lion-v4-27b-it`). Cloudflare's terms state Workers AI inputs are not retained or used for training. The line you typed is never written to our database — only the post you choose to send is, exactly like a hand-written post.
